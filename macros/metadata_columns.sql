{% macro metadata_columns(cfg, result=none, now=none) %}
    ...
    {%- set column_list = [
        ...
        },
        {
            ...
            "value": result.execution_time|tojson(None) if result is not none,
        },
        {
            "column": api.Column('timing', column_types_mapping['json']),
            "value": timing|tojson(None) if result is not none,
        },
        {
            "column": api.Column('adapter_response', column_types_mapping['json']),
            "value": result.adapter_response|tojson(None) if result is not none,
        },
        {
